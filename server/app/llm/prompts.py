...
def _agent_loop_core_instructions() -> str:
    return (
        ...
        "- `search_learning_materials` - fetch current public learning sources when the learner "
        "asks for external research, up-to-date material, or a source-grounded learning plan. "
        "Use only returned sources with URLs and fetched timestamps, and cite those URLs in the "
        "visible answer.\n"
        "- `assess_research_evidence` - after each public search, read the excerpts and make the "
        "semantic sufficiency decision yourself. Judge every required facet, cite returned source "
        "IDs, copy a short exact evidence excerpt for every covered judgment, and explain the "
        "evidence. Before the verdict, declare a task-adapted evidence standard: calibrate risk, "
        "claim types, the minimum independent sources, whether authority is required, freshness, "
        "applicability boundaries, and the stopping rule. Audit coverage, source quality, recency, "
        "counterevidence/contradictions, and applicability separately. "
        "A low-risk exploratory plan may need one excellent source; a high-stakes or disputed "
        "claim may need triangulation. The runtime checks your declared standard, provenance, and "
        "completeness only; it does not replace your judgment with a global fixed threshold or "
        "keyword matching.\n"
...
def _agent_loop_plan_instructions() -> str:
    return (
        "\n\n## Plan Skill\n"
        "- For a complex plan that depends on current external material, call "
        "`search_learning_materials` before `save_formal_plan`. Build stages from the fetched "
        "evidence, retain its source IDs in stage resources, and do not present unfetched or "
        "uncited material as verified. If retrieval is unavailable, state that boundary instead "
        "of claiming the plan is research-verified.\n"
        "- On the first formal-plan search, pass one stable `required_facets` list covering every "
        "distinct concept or decision the requested plan must substantiate, plus authoritative "
        "`preferred_domains` when known. After every search, call `assess_research_evidence`; do "
        "not let source count or literal keyword matching decide sufficiency. Continue only for "
        "the facets you judge missing. Stop when your structured verdict is `sufficient`; if it "
        "is `exhausted_with_gaps`, save an honest plan that marks those gaps instead of claiming "
        "full verification. There is no fixed normal search count: stop from evidence quality, "
        "not from round count. Long runs may compact old page bodies into a research evidence "
        "ledger; treat its source IDs, exact quotes, coverage judgments, and open gaps as the "
        "continuation checkpoint instead of restarting broad research. If a preferred domain "
        "cannot cover a remaining facet, broaden the next query to another reputable first-party "
        "or standards source instead of repeatedly fetching the same site.\n"
    ...
    *,
    ...
    if minimal_context:
        ...
        system_prompt += (
            ...
            "`search_learning_materials` first when the requested plan depends on current "
            "external sources, then call "
